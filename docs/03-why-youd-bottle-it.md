...
**Store the push in chemistry.** Two different metals in a chemical solution undergo a spontaneous reaction that separates charges — pushing positive charges to one terminal and negative to the other, maintaining the push as long as the chemical reaction continues. Volta's insight was that the reaction at the metal-fluid interface is the source of the push. Reverse the reaction (by pushing current back in the wrong direction) and you reload the chemical potential. A battery. Galvani's frog leg twitched because dissimilar metals in tissue fluid form exactly this kind of cell. Your **battery** stores much more energy than a capacitor, releases it over hours, and can (usually) be recharged.
