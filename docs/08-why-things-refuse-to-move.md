...
You have electricity and you know how to make wheels turn. But making something move isn't just about applying force — the thing you're moving pushes back in a way that has nothing to do with friction or resistance.
...
`browser/common/engine.js` tracks each `Body`'s momentum and applies `F = ma` each timestep via Euler integration. `browser/chapter08/index.html` lets you vary mass and applied force and observe the resulting acceleration. The engine separates friction (velocity-dependent clamping) from inertia (mass × acceleration) so both effects are visible independently.
...
**Continue → [Why Magnets Pull](09-why-magnets-pull.md)**
