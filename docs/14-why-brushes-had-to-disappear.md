...
- How does the torque ripple of a 3-coil BLDC compare to the DC motor from Chapter 12?

## Implementation

`browser/common/engine.js` provides the core physics; `browser/chapter14/index.html` implements electronic commutation: a position-reading loop selects the active coil pair based on rotor angle, applies the Lorentz torque, and steps the simulation. The commutation table (6 states for 3 coils) replaces the mechanical commutator from Chapter 12's sim.
...
**Continue → [Why Three Wires Are Better Than Two](15-why-three-wires-are-better-than-two.md)**
