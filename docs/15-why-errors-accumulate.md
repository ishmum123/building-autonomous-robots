...
Chapter 13 showed that feedback eliminates the effect of disturbances on speed. Chapter 14 showed you need position feedback to control position. Now you have both.
...
Keep increasing K_p: error approaches zero. But the system becomes increasingly oscillatory (Chapter 11). High K_p means a large correction for any small error — including the overshoot of trying to correct. At some gain, the system becomes unstable. You're trading steady-state accuracy for transient instability.
...
Any feed-forward that isn't updated in real time is just a one-time calibration — which is Attempt 1 from Chapter 13. And now you've added complexity (you must measure or estimate the disturbance) without solving the general problem.
...
This is why integral control (the "I" in PID) is essential for any system that must precisely hold a target against constant forces: motor control fighting gravity, temperature control fighting ambient heat loss, pressure control fighting a constant leak. Adding I to a proportional controller eliminates steady-state error — at the cost of a slower, more oscillatory response (Chapter 16 will address how to balance all three terms).
