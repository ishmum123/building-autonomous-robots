...
You have a heater you can throttle from 0–100% and a thermocouple reading temperature every 100 ms. You already know feedback control (Chapter 18): measure the error, drive the heater proportional to how far you are from 200°C. That should work. Except:
...
`browser/chapter21/index.html` models a first-order thermal plant (`plantStep`): heat input minus ambient loss drives `dT/dt`. The setpoint is 200°C. Two independent plants run side by side — P-only (ki=0) and full PID.
...
**Continue → [Why One PID Isn't Enough](22-why-one-pid-isnt-enough.md)**
