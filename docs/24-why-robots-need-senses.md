...
`browser/common/engine.js` provides `Body`, `PID`, and `Vec`. `browser/chapter24/index.html` creates two `Body` instances (`robotA` blind, `robotB` sighted) and one `PID(2.0, 0.1, 0.5, WALL_X - stopDist)`. The blind robot applies a constant `Vec(driveForce, 0)` until it hits `WALL_X`; the sighted robot calls `pidB.update(robotB.pos.x, 0.016)` each frame so it decelerates to a setpoint `stopDist` pixels from the wall. Both bodies use the same `Body.step` from the engine; the sensing logic is the PID position measurement, not a separate ground-truth layer.
...
**Continue → [Why Accelerometers Lie](25-why-accelerometers-lie.md)**
