...
`browser/common/engine.js` provides the `Quadcopter` class; `browser/chapter31/index.html` creates `qA = new Quadcopter(1.0)` and `qB = new Quadcopter(1.0)`, drives the single-motor case with `qA.setMotors(thrustLevel, 0)` and the balanced case with `qB.setMotors(thrustLevel/2, thrustLevel/2)`. The sim passes thrust directly — the thrust-vs-speed relationship is conceptual framing; the important result (balanced motors stabilize tilt) is visible in the strip plot comparing `'single motor spin°'` versus `'balanced tilt°'`.
...
**Continue → [Why Quadcopters Flip](32-why-quadcopters-flip.md)**
